-- creating databas  sales analysis - p2
...
-- List members who registed in the last 180 days
SELECT * 
FROM members
WHERE reg_date >= DATE_SUB(CURDATE(), INTERVAL 180 DAY);

-- Task.10 list employee with their branch manager's name and branch  details
select 
  e1.emp_id,
  e1.emp_name,
  e1.position,
...
-- task 12.Create a Table of Books with Rental Price Above a Certain Threshold:
...
-- Task.13 retrive the list of books not yet returned
...
-- task 13.Create a query that generates a performance report for 
-- each branch, showing the number of books issued, the number of books
